...
impl PodcastRepository {
    ...
    pub async fn upsert(&self, podcast: Podcast) -> Result<Podcast, AppError> {
    ...
    pub async fn is_subscribed_feed(&self, feed_url: &str) -> Result<bool, AppError> {
        let count =
            sqlx::query_scalar!("SELECT COUNT(1) FROM podcasts WHERE feed_url = ?", feed_url)
                .fetch_one(&self.pool)
                .await
                .map_err(|e| {
    ...
    pub async fn is_subscribed_id(&self, id: &Uuid) -> Result<bool, AppError> {
        let count = sqlx::query_scalar!("SELECT COUNT(1) FROM podcasts WHERE id = ?", id)
            .fetch_one(&self.pool)
            .await
            .map_err(|e| {
                tracing::error!("DB exists check failed: {}", e);
